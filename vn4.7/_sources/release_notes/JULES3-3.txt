...
JULES takes advantage of the parallel I/O features in `HDF5 <http://www.hdfgroup.org/HDF5/>`_ / `NetCDF4 <http://www.unidata.ucar.edu/software/netcdf/>`_. These are not enabled by default, and so must be explicitly enabled when HDF5 / NetCDF4 are compiled. More information on how to do this can be found on the `NetCDF website <https://www.unidata.ucar.edu/software/netcdf/docs/build_parallel.html>`_.
